...
#include "Sheng_Ding_persistent.h"
...
void* compute_updated_DSV(void *message) {
    ...
    for (int i = mydata->start; i <= mydata->end; i++) {
        ...
        /* Compute the updated values based on the average adjacent DSV. */
        if (average_DSV >= grid[i].box_dsv) {
            updated_DSVs[i] = grid[i].box_dsv + (average_DSV - grid[i].box_dsv) * affect_rate;
        } else if (average_DSV < grid[i].box_dsv) {
            updated_DSVs[i] = grid[i].box_dsv - (grid[i].box_dsv - average_DSV) * affect_rate;
        }
    }
...
int do_stencil_computation() {
    ...
    for (int thread_id = 0; thread_id < number_of_threads; thread_id++) {
        result  = pthread_create(&threads[thread_id], &thread_attr, converge_DSV, (void *) &message[thread_id]);
        assert(result == 0);
    }
